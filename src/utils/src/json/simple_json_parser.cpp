...
std::string SimpleJsonParser::encodeToJsonString_(JsonDataBuilder::JsonGenericParams &jsonDataMap) {
  Document document;
  document.SetObject();
...
std::string SimpleJsonParser::encodeToJsonString_(const JsonDataBuilder *jsonBuilderPtr) {
  auto data = jsonBuilderPtr->build_();
  return encodeToJsonString_(data);
}

JsonDataBuilder::JsonGenericParamsVector SimpleJsonParser::getListValues__(const rapidjson::Value::ConstArray &dataList) {
...
JsonDataBuilder::JsonGenericParams SimpleJsonParser::decodeToDataMap_(std::string &jsonString) {
  ...
  for (Value::ConstMemberIterator itr = document.MemberBegin();
    ...
    if (itr->value.IsString()) {
      ...
      Logger::log_(Logger::ERROR) << "(decodeToDataMap_) Found nesting at key (" << paramName << ")\n";
